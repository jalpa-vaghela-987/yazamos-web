...
class ProjectController extends Controller
{
    ...
    public function show(string $id)
    {
        ...
        $user = Auth::id();
        $project = Project::with(['assetType', 'phases','assignedUserProjects'])->where('id', $id)->first();
        $assignedUser = AssignedUserProject::with('user')->where('project_id', $id)->where('user_id',$user)->first();
...
//                                  ->orWhere('receiver_id', loginUser()->id);
        ...
        foreach ($phases as $phase) {
            ...
            foreach ($childrens as $children) {
                ...
                $totalActualCost += $children->actual_cost;
                $totalPlannedCost += $children->planned_cost;
            }
