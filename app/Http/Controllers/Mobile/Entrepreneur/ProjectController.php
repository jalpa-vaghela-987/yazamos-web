...
use App\Http\Resources\ProjectUserResources;
...
class ProjectController extends Controller
{
    ...
    {
        $projects = Project::with(['assetType'])->where('user_id', loginUser()->id)->orWhere('user_id', loginUser()->created_by)->get();
        if ($projects->isEmpty()) {
    ...
    {
        $project = Project::with(['assetType', 'phases','company'])->where('id', $id)->first();
        $assignedUser = AssignedUserProject::with('user')->where('project_id', $id)->where('user_id', loginUser()->id)->first();
        ...
        return response()->json([
            ...
            'data' => [
                ...
                'assignedUser' => ProjectUserResources::make($assignedUser),
                'budget_breakdown' => [
