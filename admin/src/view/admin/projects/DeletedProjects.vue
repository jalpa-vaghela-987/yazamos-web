      ...
      >
        <template #cell(actions)="row">
          <b-button
            ...
            size="sm"
            @click="restoreProject(row.item.id)"
          >
            <i class="fas fa-undo"></i> {{ $t("title.Restore") }}
          </b-button>
        </template>
      </crud-table>
    </div>
...
export default {
  ...
  methods: {
    async restoreProject(id) {
      try {
        await request({ method: "post", url: `/projects/restore/${id}` });
        this.notifySuccess(this.$t("message.ProjectRestored"));
        this.handleFilter();
      } catch (error) {
        this.notifyError(this.$t("message.SomethingWentWrong"));
      }
    },

    goToProject(id) {
      this.$router.push({
        name: this.$global.hasRole("entrepreneur") ? "ProjectShow" : "AdminProjectShow",
        params: { id },
      });
    },
    editProject(id) {
      this.$router.push({
        name: this.$global.hasRole("entrepreneur") ? "ProjectEdit" : "AdminProjectEdit",
        params: { id },
      });
    },
    openModal() {
